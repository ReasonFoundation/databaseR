...
,2.58
,1.54)
treasury <- data.table(t(treasury))

treasury <- data.table(treasury[, year := NA])
treasury$year <- seq(2001, 2020, by = 1)
...
ui <- fluidPage(
  ...
  titlePanel("State Pension 2001-20 Returns"),
...
server <- function(input, output, session){
  ...
  output$plot_Return_Distribution <- plotly::renderPlotly({
    ...
    returns <- data.frame(returns_2020)
    
    }else{
    
    returns <- reason.data %>% select(year, plan_name, return_1yr)
    returns <- returns %>% filter(year == input$year2)
   ...
   # View(returns)
    ...
    annotation <- list(yref = 'paper', xref = "x", y = 0.5, x = median(round(na.omit(returns[,3]),2)), text = paste("Median = ", round(median(na.omit(returns[,3]))*100,1), "%"))
    #https://plotly.com/chart-studio-help/histogram/
    d1 <- density(na.omit(returns[,3]), bw = "SJ")
    
    quantile <- quantile(na.omit(returns[,3]), c(0.25, 0.5, 0.75))
    
    histogram <- plot_ly(x = na.omit(returns[,3]),
      ...
      #add_boxplot(x = na.omit(returns[,3]), y =  d1, jitter = 0, pointpos = 0, boxpoints = F,
  ...
  #https://stackoverflow.com/questions/49636423/how-to-change-the-cell-color-of-a-cell-of-an-r-shiny-data-table-dependent-on-it
   ...
   }else if (input$lines == "30Y Treasury Yield"){
     p <- add_lines(p, x = trace5$x, 
                    xend =  max(trace5$x), y = round(treasury[year >= min(input$year,2018)]$V1,2)/100, 
                    yend = last(round(treasury[year >= min(input$year,2018)]$V1,2))/100, showlegend = T,
                    name = "30Y Treasury", line = list(color = palette_reason$Yellow))
   }else{NULL}
  #  
