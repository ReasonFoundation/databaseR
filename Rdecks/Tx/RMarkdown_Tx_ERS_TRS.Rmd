...
```{r debtTRS, dpi=400, echo = FALSE, fig.cap = "Source: Pension Integrity Project analysis of Texas TRS actuarial valuation reports.", fig.width=8, fig.height=5, fig.align = "center", message=FALSE, warning=FALSE}
...
```{r debt2, dpi=400, echo = FALSE, fig.cap = "Source: Pension Integrity Project analysis of Texas TRS actuarial valuation reports.", fig.width=8, fig.height=5, fig.align = "center", message=FALSE, warning=FALSE}
...
```{r area.ual, dpi=400, echo = FALSE, fig.cap = "Source: Pension Integrity Project analysis of Texas TRS actuarial valuation reports.",fig.width=8, fig.height=5, fig.align = "center", message=FALSE, warning=FALSE}
...
area.ual <- ualPlot(Data, title = NULL, caption = FALSE, grid = FALSE,
                 ticks = TRUE, font = "Calibri",
                  yaxisMin = 0, yaxisMax = 250, yaxisSeq = 25,
...
```{r, dpi=400, echo = FALSE, caption = "Source: Pension Integrity Project analysis of Texas TRS actuarial valuation reports.",fig.width=8, fig.height=5, fig.align = "center", message=FALSE, warning=FALSE}
...
url = "https://raw.githubusercontent.com/ReasonFoundation/databaseR/master/Rdecks/Tx/TxTRS_GL.csv" 
urlfile=url
...
glPlot <- function(database = FALSE, data = NULL,
      url = "https://raw.githubusercontent.com/ReasonFoundation/databaseR/master/apps/Texas TRS_GL.csv",
      interactive = TRUE,
...
### Driving Factors Jeopardizing Texas TRS Resiliency

-   **Deviations from Investment Return Assumptions** have been the largest contributor to the unfunded liability growth, adding **$`r inv`** billion from 2002 to 2021.

    </p>

-   **Changes to Actuarial Methods and Assumptions** – including assumed rate of return - have revealed roughly **$`r assum`** billion in additional unfunded liability since 2002.

    </p>

-   **Extended Amortization Timetables and low Statutory Contributions** in the past have resulted in interest on Texas TRS debt exceeding the actual debt payments (negative amortization) since 2001, adding a net **$`r amo`** billion to the unfunded liabilities. 

    </p>

-  **Deviations from Demographic and Pay Increase Assumptions** – including deviations from assumed rates of withdrawal, retirement, mortality, payroll increase — added **$`r demo`** billion to the debt since 2002.
...
```{r graph, dpi = 400, echo = FALSE, fig.cap = "Source: Pension Integrity Project analysis of Texas TRS actuarial valuation reports.", fig.width=8, fig.height=5, fig.align = "center", message=FALSE, warning=FALSE}
### linePlot ###
...
Data<- Data %>% filter (plan_name == "Texas Teacher Retirement System")
#View(Data)
Data$year <- as.numeric(Data$year)
#Set to data.frame for visualization
Data <- data.frame(Data)
...
graph <- linePlot(Data, title = "", 
                  ...
                  treasury = FALSE,
                  font = "Calibri", 
                  yaxisMin = -24, yaxisMax = 28, yaxisSeq = 4,
...
```{r sp500, dpi=400, echo = FALSE, fig.cap = "Source: Pension Integrity Project analysis of Texas TRS actuarial valuation reports.", fig.width=8, fig.height=5, fig.align = "center", message=FALSE, warning=FALSE}
...
3.  As Texas TRS waits for the continual "rebound" its unfunded liabilities continue to grow.

### TxTRS Asset Allocation (2001-2021) w/ R (custom)

```{r assets2, dpi=400, echo = FALSE, fig.cap = "Source: Pension Integrity Project analysis of Texas TRS actuarial valuation reports.", fig.width=8, fig.height=5, fig.align = "center", message=FALSE, warning=FALSE}
...
assets <- filterAssets2(data = assets, fy = 2001, plan = "Texas Teacher Retirement System")
...
assets <- data.table(assets)

# #### Assigning any gap w/ 100% to Equities
# data <- read_csv(url(
#           "https://raw.githubusercontent.com/ReasonFoundation/databaseR/master/files/TxERS_Allocation_PrEquity.csv"), 
#           col_names = TRUE, na = c(""), skip_empty_rows = TRUE, col_types = NULL)
#         
#         data <- data.frame(data)
#    data <- data %>% replace(is.na(.), 0)
...
#  assets$`Fixed Income` <- data$Fixed.Income
#         assets$`Hedge Funds` <- data$Hedge.Funds
#         assets$`Real Estate` <- data$Real.Estate
#         assets$`Other Alternatives` <- data$Other.Alternatives
#         assets$`Total Equity` <- data$Total.Equity
#         assets$`Cash Equivalents` <- data$Cash.Equivalents
#         assets$`Private Equity` <- data$Private.Equity
#         assets$Commodities <- data$Commodities
#         
...
#View(assets)
assets2 <- assetPlot(assets, interactive = FALSE) 
#View(assets)
assets2
```
...
```{r table, dpi=400, echo = FALSE, fig.cap = "Source: Pension Integrity Project Monte Carlo model based on Texas TRS asset allocation and reported expected returns by asset class. Forecasts of returns by asset class generally by BNYM, JPMC, BlackRock, Research Affiliates, and Horizon Actuarial Services were matched to the Texas TRS specific asset class. Probability estimates are approximate as they are based on the aggregated return by asset class. For complete methodology contact Reason Foundation.", fig.width=8, fig.height=5, fig.align = "center", message=FALSE, warning=FALSE}
...
FileName <- "TxTRS_Return_Data.xlsx"
NumberofYears <- 20 
NumberofSimulations <- 100000

Percentiles <- c(5,10,25,50,75,90,95)
Probabilities <- c(9,8,7.25,7,6.5,6,5)
FundARR <- 0.0725
#
...
colnames(table) <- c("Possible Rates of Return",
                     "Based on Texas TRS Assumptions",
                     "Texas TRS Historical Returns",
...
### Texas TRS Unfunded Liability Sensitivity to DR Changes 2021 w/ R (flextable())
...
sensitivity$npl_pct_up <- 979861293
### Rediscount function
...
#Use database numbers to populate/calculate needed values
table[1:4,1] <- c(seq(0.0725,0.065, by = -0.0025))
...
colnames(table) <- c("Discount Rate", 
                  "Funded Ratio (MVA)", 
                  "Unfunded Liability (MVA) in $Billions", 
                  "Actuarial Accrued Liability in $Billions")

#Format data to %/$ for appearance
table[1:4,1] <- scales::percent(table$`Discount Rate`, accuracy = 0.01)
...
```{r net.amo, dpi=400, echo = FALSE, fig.cap = "Source: Pension Integrity Project analysis of Texas TRS actuarial valuation reports.", fig.width=8, fig.height=5, fig.align = "center", message=FALSE, warning=FALSE}
  ...
  lab9 = NULL) 
{
  ...
  if (isTRUE(net.amo)) {
    ...
    reason.data$interest_on_debt_dollar <- as.numeric(reason.data$interest_on_debt_dollar)+as.numeric(reason.data$gain_or_loss_due_to_other_interest)
    reason.data$net.amo <- 0
    reason.data$amo.payments <- 0
    reason.data$amo.payments <- as.numeric(reason.data$total_contribution_dollar) - as.numeric(reason.data$gain_or_loss_due_to_normal_cost_prior_year)
...
pl <- planList()
reason.data <- pullData(pl, "Texas Teacher Retirement System") %>% filter(year > 2000 & !duplicated(year))
#View(reason.data)
#(2002-2020)
...
```{r contributions, dpi=400, echo = FALSE, fig.cap = "Source: Pension Integrity Project analysis of Texas TRS actuarial valuation reports.", fig.width=8, fig.height=5, fig.align = "center", message=FALSE, warning=FALSE}
...
#reason.data[1:3,3:4] <-  reason.data[1:3,2]
    ...
    graph <- ggplot(reason.data) + 
      ...
      size = 3)) + scale_y_continuous(labels = function(x) paste0("$", 
      x/yaxisScale), name = if (isTRUE(net.amo)) {
      "Unfunded Liability Payments (in $Billions)"
    }
    else {
      "Employer Contributions v. ADC (in $Millions)"
    }, breaks = seq(Ymin, Ymax, by = Ybreak), limits = c(Ymin, 
      ...
      ggplot2::theme(axis.ticks.x = element_line(size = 0.5, 
        color = "black")) + ggplot2::theme(axis.ticks.y = element_line(size = 0.5, 
      color = "black")) + scale_x_continuous(labels = function(x) paste0(x, 
      ""), name = "", breaks = seq(1996, 2022, by = 2), 
      limits = c(1995, 2022), expand = c(0, 0)) + theme_bw() + theme(axis.line = element_line(), panel.grid.major = element_blank(), 
      panel.grid.minor = element_blank(), panel.border = element_blank(), 
      panel.background = element_blank()) + theme(legend.position = c(0.25, 
      0.8))+ plotTheme 
  }
...
#Pull plan-specific data from the database
pl <- planList()
reason.data <- pullData(pl, "Texas Teacher Retirement System") %>% filter(year > 2000 & !duplicated(year))# (2002-2020)
...
data <- data.frame(year = seq(1996, 2017, by = 1),
                   arc_adec_paid_dollar = 
c(899129,
956667,
1022713,
1086238,
...
x <- Data %>% full_join(data, by = c("year"))


Data <- x


Data <- Data %>% transmute( year = year,
      arc_adec_dollar = arc_adec_dollar.y, 
      statutory = arc_adec_paid_dollar.y,
      adec_paid = arc_adec_paid_dollar.y,
      adec_unpaid = (arc_adec_dollar - adec_paid)) %>%
  arrange(year)

Data$arc_adec_dollar[23:26] <- x$arc_adec_dollar.x[18:21]/1000
Data$adec_paid[23:26] <- x$statutory[18:21]/1000
Data$statutory <- Data$adec_paid
Data$adec_unpaid[23:26] <- as.numeric(Data$arc_adec_dollar[23:26]) - as.numeric(Data$adec_paid[23:26])
...
### Texas TRS Amortization Period (2001-2021) custom R

```{r amo.period, dpi=400, echo = FALSE, fig.cap = "Source: Pension Integrity Project analysis of Texas TRS actuarial valuation reports.", fig.width=8, fig.height=5, fig.align = "center", message=FALSE, warning=FALSE}
...
Data$amo.period[20:21] <- c(27,23)
Data$amo.period[2:4] <- 50#2001-2004 period is 100 (lowering it to see the amo line)
Data$amo.period[1] <- 0
...
########areaPlot
  ...
  amo.period <- ggplot2::ggplot(data, ggplot2::aes(x = year)) + 
    ...
    ggplot2::scale_y_continuous(breaks = seq(0, 50, by = 5),limits = c(0, 50.5), expand = c(0, 0))+
...
```{r treasury, dpi=400, echo = FALSE, fig.cap = "Source: Pension Integrity Project analysis of Texas TRS actuarial valuation reports.", fig.width=8, fig.height=5, fig.align = "center", message=FALSE, warning=FALSE}
...
```{r payroll, dpi=400, echo = FALSE, fig.cap = "Source: Pension Integrity Project analysis of Texas TRS actuarial valuation reports. Actual payroll does not include payroll for members in ORP or working retirees.", fig.width=8, fig.height=5, fig.align = "center", message=FALSE, warning=FALSE}
  ...
  lab3 = NULL, lab4 = NULL, lab5 = NULL) 
{
  ...
  data <- as.data.table(data)
  graph <- data.table(melt(data, id.vars = "year"))
  lineColors <- c(palette_reason$SatBlue, palette_reason$Yellow, 
...
payroll <- linePlot(Data, title = NULL, caption = FALSE, grid = FALSE, 
                  ...
                  yaxisMin = 0, yaxisMax = 60, yaxisSeq = 6,
                  yaxisScale = 1/1000000000, format = "$", str = 60,
...
```{r inflation, dpi=400, echo = FALSE, fig.cap = "Source: Pension Integrity Project analysis of Texas TRS actuarial valuation reports.", fig.width=8, fig.height=5, fig.align = "center", message=FALSE, warning=FALSE}
...
Data<- Data %>% filter(plan_name == "Texas Teacher Retirement System") %>% filter(!duplicated(year))
Data$year <- as.numeric(Data$year)
Data <- data.frame(Data)
...
inflation <- linePlot(Data, title = NULL, caption = FALSE, grid = FALSE, 
                  ...
                  yaxisScale = 100, format = "%", str = 60,
                  labelY = "Inflation %", lab1 = "Assumed Inflation (by Texas TRS)",
...
```{r int.debt, dpi=400, echo = FALSE, fig.cap = "Source: Pension Integrity Project analysis of Texas TRS actuarial valuation reports.", fig.width=8, fig.height=5, fig.align = "center", message=FALSE, warning=FALSE}
...
reason.data <- pullData(pl, "Texas Teacher Retirement System") %>% filter(year > 2000 & !duplicated(year))
### Net Amo
reason.data$x1_year_investment_return_percentage

 reason.data$interest_on_debt_dollar <- -(as.numeric(reason.data$interest_on_debt_dollar)+as.numeric(reason.data$gain_or_loss_due_to_other_interest))
    reason.data$net.amo <- 0
    reason.data$amo.payments <- 0
    reason.data$amo.payments <- as.numeric(reason.data$gain_or_loss_due_to_normal_cost_prior_year)-as.numeric(reason.data$total_contribution_dollar)
...
reason.data$compound[2] <- reason.data$net.amo[2]
...
### Benefit Accrual & Retention for an Average New Texas TRS Employee (Age 27)  w/ R (custom)

```{r pwealth, dpi=400, echo = FALSE, fig.cap = "Source: Pension Integrity Project analysis of Texas TRS actuarial reports and CAFRs. Illustration is based on plan’s Class Three assumptions and a hypothetical analysis of an average new member hired at the age of 27", fig.width=8, fig.height=5, fig.align = "center", message=FALSE, warning=FALSE}
...
### Delaying Eligibility for Full Benefits Backloads `r entry` Age New Texas TRS Hires' Benefits

Only **$`r pw.28`K** is what new Age `r entry` (Class Three) Texas TRS members will accrue after 28 years on the job (accrued benefits valued at retirement).
...
### Retention Rates for a New Texas TRS Employee  w/ R (custom)
...
### Retention: Only Few of `r entry` Age New Texas TRS Hires Receive Benefits

Only **`r vest.prob*100`%** of new Age `r entry` (Class Three) Texas TRS members will stay for 8 years.
