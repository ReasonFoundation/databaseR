...
View(pl)
PERSI.new <- filteredData(pl, "Idaho Public Employee Retirement System", 2001) %>% arrange(year)
PERSI.prev <- data.table(reason.data[plan_name == "Idaho Public Employee Retirement System"])%>% arrange(year)
...
#RUN THIS:
pl <- planList() #Save list of all plans in the database
#Uncomment below to downlod data

#View(pl)
#plan <- pullData(pl, "CalPERS - California Public Employees Retirement Fund")
#View(plan)
...
#Download state-level data directly from the database
#reason.data  <- data.table(
#      pullData(pl, plan.names$state.plans.nm) %>%
#      arrange(state, year))
#reason.data <- reason.data %>% filter(administering_government_type == 0) #To filter for state-level plans
#View(plan.names)

#Add plan name abbreviations 
#for (i in 1:reason.data[,.N]){
#  reason.data[display_name %in% plan.names[i,1]]$display_name <- as.character(plan.names[i,2])
#}
...
server <- function(input, output, session){
  ...
  PlanSourceData <- reactive({
    ...
    pullPlanData <- function(plan_name)
    ...
    if(input$filter == "Filtered"){
      Plan <- data.table(pullPlanData(input$y))
      Plan <- filteredSourceData(Plan, input$year)
    } else {
      Plan <- data.table(pullPlanData(input$y)) %>%
        filter(year >= input$year)
    } 
